...
- 샵 정보(영업시간, 가격, 시술 소요시간, 예약금, 계좌번호, 정책) 기반 LLM 1차 답변 시도
- 샵 정보로 답변 가능한 경우 즉시 응답 생성
- 샵 정보에 없는 내용은 `notify_owner()` SSE 알림 전송 후 대기 안내 (human-in-the-loop)
- "기타" 단어 입력 시에도 inquiry로 분류하여 처리
...
3. LLM이 샵 정보 기반으로 답변 생성 시도
4. 정보가 없는 경우 사장님에게 SSE 알림 전송 후 대기 안내 반환
5. 고객에게 응답 반환
